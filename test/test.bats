...
@test "Good fasta file should return 0" {
  run ./fa-lint -fasta test/fasta/good.fasta
  echo "$output"
...
@test "Good mixed fasta file should return 0" {
  run ./fa-lint -fasta test/fasta/good_mixed.fasta
  echo "$output"
  [ "$status" -eq 0 ]
}

@test "Good multiline fasta file should return 0" {
  run ./fa-lint -fasta test/fasta/good_multiline.fasta
  echo "$output"
  [ "$status" -eq 0 ]
}
...
@test "Fasta file with no > on the first line should fail" {
  run ./fa-lint -fasta test/fasta/no_first_line.fasta
  echo "$output"
